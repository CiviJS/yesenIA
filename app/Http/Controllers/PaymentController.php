...
class PaymentController extends Controller
{
    ...
    public function cancelPay(Payment $payment){
        try{
            $this->paymentService->cancelPay($payment);
            return redirect()->back()->with('success', 'pago eliminado correctamente');
        }catch(Exception $e) {
            Log::info('error critico al eliminar pago: ' . $e);
            return redirect()->back()->with('error', 'Ups! hubo un error al eliminar el pago, intente nuevamente');
        }
    }
}
